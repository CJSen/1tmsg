...
function loadConfig() {
  if (!existsSync(CONFIG)) {
    copyFileSync(PLAIN_TEMPLATE, CONFIG);
    console.log(`[config] 已生成 ${basename(CONFIG)} —— 不带图片的版本`);
    console.log(`[config] 想发图片：cp ${basename(R2_TEMPLATE)} ${basename(CONFIG)}（需先开通 R2）`);
  }
  try {
    return JSON.parse(stripTrailingCommas(stripJsonc(readFileSync(CONFIG, 'utf8'))));
  } catch (err) {
    throw new Error(`${basename(CONFIG)} 不是合法 JSON：${err.message}`);
  }
}
...
/**
 * 部署默认语言：读 wrangler.jsonc 的 vars.DEFAULT_LOCALE。
 *
...
export function defaultLocale() {
  ...
  throw new Error(
    `wrangler.jsonc 的 vars.DEFAULT_LOCALE 只能是 ${LOCALES.map((l) => `"${l}"`).join(' 或 ')}，当前是 ${JSON.stringify(raw)}`,
  );
}
 ...
 *
 * 由 scripts/feature-flag.mjs 依据 wrangler.jsonc 的 vars.DEFAULT_LOCALE 生成，
 * 调用方：scripts/build-client.mjs 与 npm run typecheck（scripts/sync-locale.mjs）。
 *
 * 当前默认语言：${locale}
 * 改动方式：改 wrangler.jsonc 的 vars.DEFAULT_LOCALE，然后重新构建。
 */
